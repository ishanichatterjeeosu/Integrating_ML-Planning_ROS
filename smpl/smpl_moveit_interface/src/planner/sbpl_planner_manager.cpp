...
{
    ROS_INFO_NAMED(PP_LOGGER, "Motion Plan Request");

    ROS_INFO_NAMED(PP_LOGGER, "  workspace_parameters");
    ROS_INFO_NAMED(PP_LOGGER, "    header");
    ROS_INFO_NAMED(PP_LOGGER, "      seq: %u", req.workspace_parameters.header.seq);
    ROS_INFO_STREAM_NAMED(PP_LOGGER, "      stamp: " << req.workspace_parameters.header.stamp);
    ROS_INFO_NAMED(PP_LOGGER, "      frame_id: \"%s\"", req.workspace_parameters.header.frame_id.c_str());
    ROS_INFO_NAMED(PP_LOGGER, "    min_corner");
    ROS_INFO_NAMED(PP_LOGGER, "      x: %f", req.workspace_parameters.min_corner.x);
    ROS_INFO_NAMED(PP_LOGGER, "      y: %f", req.workspace_parameters.min_corner.y);
    ROS_INFO_NAMED(PP_LOGGER, "      z: %f", req.workspace_parameters.min_corner.z);
    ROS_INFO_NAMED(PP_LOGGER, "    max_corner");
    ROS_INFO_NAMED(PP_LOGGER, "      x: %f", req.workspace_parameters.max_corner.x);
    ROS_INFO_NAMED(PP_LOGGER, "      y: %f", req.workspace_parameters.max_corner.y);
    ROS_INFO_NAMED(PP_LOGGER, "      z: %f", req.workspace_parameters.max_corner.z);
    ...
    for (size_t cind = 0; cind < req.goal_constraints.size(); ++cind) {
        ...
        // joint constraints
        ROS_INFO_NAMED(PP_LOGGER, "    joint_constraints: %zu", constraints.joint_constraints.size());
        ...
        ROS_INFO_NAMED(PP_LOGGER, "    position_constraints: %zu", constraints.position_constraints.size());
        for (size_t pcind = 0; pcind < constraints.position_constraints.size(); ++pcind) {
            ...
            ROS_INFO_NAMED(PP_LOGGER, "      header: { frame_id: %s, seq: %u, stamp: %0.3f }", pos_constraint.header.frame_id.c_str(), pos_constraint.header.seq, pos_constraint.header.stamp.toSec());
            ROS_INFO_NAMED(PP_LOGGER, "      link_name: %s", pos_constraint.link_name.c_str());
            ROS_INFO_NAMED(PP_LOGGER, "      target_point_offset: (%0.3f, %0.3f, %0.3f)", pos_constraint.target_point_offset.x, pos_constraint.target_point_offset.y, pos_constraint.target_point_offset.z);
            ROS_INFO_NAMED(PP_LOGGER, "      constraint_region:");
            ROS_INFO_NAMED(PP_LOGGER, "        primitives: %zu", pos_constraint.constraint_region.primitives.size());
    ...
    ROS_INFO_NAMED(PP_LOGGER, "  allowed_planning_time: %f", req.allowed_planning_time);
...
void LogPlanningScene(const planning_scene::PlanningScene& scene)
{
    ...
    ROS_INFO_NAMED(PP_LOGGER, "  Has Collision Env: %s", scene.getCollisionEnv() ? "true" : "false");
    ...
    PlannerConfigurationMap* pcm_out)
{
    ...
    //ROS_INFO_NAMED(PP_LOGGER, "Successfully loaded planner settings, '%s', '%s', '%s', '%s'", (search_settings["ARAConfig"]["search"]).c_str(), (heuristic_settings["BFSConfig"]["bfs_res_x"]).c_str(), (graph_settings["RightArmManipLatticeConfig"]["discretization"]).c_str(), (shortcut_settings["JointSpaceShortcutConfig"]["shortcutter"]).c_str()); getchar();
...
auto SBPLPlannerManager::getPlanningContext(
    ...
    -> PlanningContextPtr
{
    ROS_INFO_NAMED(PP_LOGGER, "Get SBPL Planning Context");
...
bool SBPLPlannerManager::canServiceRequest(const MotionPlanRequest& req) const
{
    ...
    //ROS_INFO_NAMED(PP_LOGGER, "min max corner %f", req.workspace_parameters.max_corner.x); 

    if (min_corner.x > max_corner.x ||
        min_corner.y > max_corner.y ||
        min_corner.z > max_corner.z)
    {
        //ROS_WARN_NAMED(PP_LOGGER, "min max corner %d", req.workspace_parameters.min_corner); 
