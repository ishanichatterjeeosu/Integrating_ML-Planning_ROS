...
#include "../collision/collision_env_sbpl.h"
...
namespace moveit_msgs
{
    ...
    static auto to_cstring(moveit_msgs::MoveItErrorCodes code) -> const char *
    {
        switch (code.val)
        {
...
namespace sbpl_interface
{
    ...
    static bool InitPlanningParams(
        const std::map<std::string, std::string> &config,
        smpl::PlanningParams *pp);

    static bool InitGridParams(
        const std::map<std::string, std::string> &config,
        double *grid_res_x,
        double *grid_res_y,
        double *grid_res_z,
        ...
        moveit_msgs::MotionPlanRequest &req)
    {
        ...
        // ROS_INFO_NAMED(PP_LOGGER, "workspace params, '%s'", req_in.workspace_parameters.header.frame_id.c_str()); getchar();
        ...
        -> std::unique_ptr<smpl::OccupancyGrid>
    {
        ...
        auto cworld = scene.getCollisionEnv();

        using collision_detection::CollisionEnvSBPL;
        auto *sbpl_cworld = dynamic_cast<const CollisionEnvSBPL *>(cworld.get());
    ...
    SBPLPlanningContext::SBPLPlanningContext(
        ...
        : Base(name, group),
          ...
          m_planner()
    {
        ROS_INFO_NAMED(PP_LOGGER, "Constructed SBPL Planning Context");
    }
    ...
    bool SBPLPlanningContext::solve(planning_interface::MotionPlanResponse &res)
    {
        ...
        // ROS_INFO_NAMED(PP_LOGGER, "      frame_id: \"%s\"", robot->getModelFrame().c_str()); getchar();
        ...
        // SET FRAME ID AND ALLOWABLE PLANNING TIME
        req_msg.workspace_parameters.header.frame_id = robot->getModelFrame();
        req_msg.allowed_planning_time = 10.0;
        ...
        ROS_INFO_NAMED(PP_LOGGER, "updated req frame id: %s", req_msg.workspace_parameters.header.frame_id.c_str());
        ROS_INFO_NAMED(PP_LOGGER, "allowed planning time: %f", req_msg.allowed_planning_time);
        ...
        ROS_INFO_NAMED(PP_LOGGER, "Solve!");
